...
class RealTradingSimulator:
    ...
    def __init__(self, symbol: str = "BTCUSDT"):
        ...
        # Phase C 決策引擎（使用更寬鬆的參數以增加交易機會）
        self.trading_engine = LayeredTradingEngine(
            signal_config={
                'long_threshold': 0.4,    # 降低: 0.6 → 0.4 (允許較弱信號)
                'short_threshold': 0.4
            },
            regime_config={
                'vpin_threshold': 0.65,   # 放寬: 0.5 → 0.65
                # VPIN > 0.7 = CRITICAL (阻擋)
                # VPIN 0.65-0.7 = DANGER (阻擋)
                # VPIN 0.3-0.65 = WARNING (允許) ✅
                # VPIN < 0.3 = SAFE (允許)
            }
        )
