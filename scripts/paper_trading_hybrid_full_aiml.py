...
class HybridPaperTradingSystem:
    ...
    def __init__(
        ...
        # 🆕 動態策略配置系統
        # Allow custom config via env (TRADING_CONFIG_PATH); default to AI artifacts copy if present
        custom_cfg = os.environ.get("TRADING_CONFIG_PATH")
        default_ai_cfg = os.path.join(os.path.dirname(__file__), "..", "ai_dev", "artifacts", "trading_strategies_dynamic_ai.json")
        cfg_path = custom_cfg or default_ai_cfg if os.path.exists(default_ai_cfg) else "config/trading_strategies_dynamic.json"
